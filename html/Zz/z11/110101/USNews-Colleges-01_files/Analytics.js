...
for (var i=0; i < metas.length; i++) {
	if (metas[i].name == "citation")  {
		citation = metas[i].content;
	}
	else citation = "Uncategorized";
	if (metas[i].name == "source")  {
		source = metas[i].content;
	}
	else source = "Uncategorized";
}
if (metas.length == 0) {
	source = citation = "Uncategorized";
}
/*  USN Code                                              */
...
if (window.location.host.indexOf('health') >= 0 ||
    window.location.host.indexOf('politics') >= 0 ||
    window.location.host.indexOf('money') >= 0) {
...
/*s.prop1=""
	usn code
	s.prop2=""
	if (location.pathname.match("/articles/") || location.pathname.match("/archive/")) {
	s.prop2=location.pathname.substring(location.pathname.lastIndexOf('/')+1,location.pathname.lastIndexOf('.'));
	} */
...
// grab and pass bylines --PROTOTYPE-SPECIFIC!!--
s.prop4='';
$$(".byline a").each(function (a) {
        if (s.prop4=='') s.prop4=a.innerHTML;
});
$$("#byline a").each(function (a) {
        if (s.prop4=='') s.prop4=a.innerHTML;
});
...
if (window.gaCalled == null) {
  ...
  default: // Assume hostname == *.usnews.com.
    ...
    if (window.USN) {
      // We're using James' code!
      firstAccount.USN_Site = USN.site;
      firstAccount.USN_Zone = USN.zone;
    } else {
      // We're supporting a legacy page (e.g., for Prototype 1.5, *.usnews.com/usnews/* stuff).
      // firstAccount.USN_Site = $$('meta[name=site]')[0].content;
      // firstAccount.USN_Zone = $$('meta[name=zone]')[0].content;
...
// handlers for usnud links

$$("a").each(function (a) {
        Event.observe(a,'click',function() {
                if (a.href.indexOf('video.healthination.com') >= 0) {
                        s.eVar21 = a.href.substring(a.href.lastIndexOf('/')+1,a.href.lastIndexOf('.'));
                }
        });
});

$$("a.usnud").each(function (a) {
	Event.observe(a,'click',function () {
        	var s=s_gi('usnews');
        	s.linkTrackVars='None';
        	s.linkTrackEvents='None';
...
$$("a.udnudw-submitButton").each(function (a) {
	Event.observe(a,'click',function() {
        	var s=s_gi('usnews');
        	s.linkTrackVars='None';
        	s.linkTrackEvents='None';
...
if ($('#axa-sharebar-link')) {
    $('#axa-sharebar-link').observe('click', function () {
        var s=s_gi('usnews');
        s.linkTrackVars='None';
        s.linkTrackEvents='None';
...
/************* Rubicon **************/
var _rub = "%3Cscript type='text/javascript' defer='defer' src='http://tap-cdn.rubiconproject.com/partner/scripts/rubicon/alice.js?pc=7715/12318&autorun=true'%3E%3C/script%3E";
document.write(unescape(_rub));
